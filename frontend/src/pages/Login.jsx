  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--color-ink)' }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative items-center justify-center overflow-hidden"
        style={{ background: 'linear-gradient(135deg, var(--color-burgundy-dark), var(--color-ink))' }}>
        ...
        <div className="relative z-10 text-center px-12">
          ...
          <h2 className="text-4xl mb-4" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-cream)' }}>
            Welcome Back
          </h2>
          <p className="text-lg" style={{ color: 'var(--color-cream)', opacity: 0.6 }}>
            Your curated library awaits.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8" style={{ background: 'var(--color-parchment)' }}>
        <div className={`w-full max-w-md ${shaking ? 'shake' : ''}`}>
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-ochre)' }}>Enter</span>
          </div>
          <h1 className="text-4xl mb-2" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-ink)' }}>Sign In</h1>
          <p className="mb-8 text-sm" style={{ color: 'var(--color-ink)', opacity: 0.5 }}>Access your digital library.</p>

          {error && (
            <div className="mb-6 p-3 rounded-md text-sm" style={{ background: 'rgba(107,30,46,0.1)', color: 'var(--color-burgundy)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>
              {error}
            </div>
          )}
          ...
          <div className="mt-6 flex items-center gap-4">
            <div className="flex-1 h-px" style={{ background: 'var(--color-mist)' }} />
            <span className="text-xs" style={{ fontFamily: 'var(--font-mono)', opacity: 0.4, color: 'var(--color-ink)' }}>or</span>
            <div className="flex-1 h-px" style={{ background: 'var(--color-mist)' }} />
          </div>

          <button className="w-full mt-6 py-3 rounded-md text-sm border transition-colors" data-cursor="button"
            style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', background: 'transparent', borderColor: 'var(--color-mist)', color: 'var(--color-ink)' }}>
            Continue with Google
          </button>

          <p className="mt-8 text-center text-sm" style={{ color: 'var(--color-ink)', opacity: 0.5 }}>
            Don't have an account?{' '}
            <Link to="/register" className="no-underline font-medium" style={{ color: 'var(--color-ochre)' }} data-cursor="button">Create one</Link>
          </p>
        </div>
      </div>
    </div>
